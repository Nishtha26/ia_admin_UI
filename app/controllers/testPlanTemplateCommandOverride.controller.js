    ...
    function ($scope, $compile, $rootScope, $location, AppServices, GraphServices, GraphMaximizeServices,
        ...
        if (messagesTemplate.length == 1) {
            for (var i = 0; i < messagesTemplate[0].length; i++) {
                ...
                if (messagesTemplate[0][i].key == "customers")
                    $scope.customers = messagesTemplate[0][i].value;
            }
        }
...
//			$("#updateCommandParametersForm").append('<div><input name="command[' + commandIndex + '].Name" type="text" value="" /></div><br/>');
        ...
        promise2.then(
            function (data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    ;
                    jsonArray.push({
                        id: VirtualDevice[i].name,
                        deviceId: VirtualDevice[i].id,
                        content: jQuery.extend(true, new Object(), $scope.uiTreeJSON)
                    });
                }
        ...
        $scope.saveTestPlan = function () {
            ...
            sendCreateData.isTestPlanTemplate = 'Y';
